...
Contact costs health. Reach zero and the run is over — **clear all ten rounds and you win**,
with your score and remaining health on the end screen.
...
bullet speed, fire cooldown, contact damage, per-round heal, and `finalRound` — the wave that ends
the run. Enemy count (`round*2+2`), speed and hit points scale in `nextRound`.
